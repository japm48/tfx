...
class StepBuilder:
  ...
  def __init__(self,
               ...
               dynamic_exec_properties: Optional[Dict[str, Dict[Any,
                                                                Any]]] = None,
  ...
  def build(self) -> Dict[str, pipeline_pb2.PipelineTaskSpec]:
    ...
    for name, output_channel in self._outputs.items():
      ...
      # artifact. cl/456339615 supports component parameters as outputs
      # and inputs.

      if self._dynamic_exec_properties and self._node.id in self._dynamic_exec_properties:
        if name in self._dynamic_exec_properties[self._node.id]:
          component_def.output_definitions.parameters[
              name].parameter_type = standard_artifacts.String(
                  self._dynamic_exec_properties[self._node.id][name])
    ...
    for name, input_channel in itertools.chain(self._inputs.items(),
      ...
      input_parameter_spec.task_output_parameter.producer_task = input_channel.producer_component_id + '_task'
      input_parameter_spec.task_output_parameter.output_parameter_key = input_channel.output_key
