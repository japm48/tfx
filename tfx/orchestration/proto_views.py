...
"""Helps read a node or a subpipeline's info from its proto representation."""
...
class ComposablePipelineProtoView(NodeProtoView):
  ...
  def __init__(self, pipeline: pipeline_pb2.Pipeline):
    ...
    assert len(self._pipeline.nodes) >= 2, (
        'Got malformed subpipeline proto. Expecting at least two nodes '
        '(PipelineBegin node and PipelineEnd node) in the subpipeline.')
    ...
    ), ('Got malformed subpipeline proto. '
        'Expecting the first node in subpipeline to be a PipelineBegin node.')
    ...
    assert self._end_node.node_info.type.name.endswith(
        compiler_constants.PIPELINE_END_NODE_SUFFIX), (
            'Got malformed subpipeline proto. '
            'Expecting the last node in subpipeline to be a PipelineEnd node.')
